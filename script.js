...
function setup() {
  select('body').style('font-family', 'Lato');
  textFont('Lato');
...
function startVisualizer() {
  ...
  let customFileButton = createButton('Change Song'); // Create a custom button
  customFileButton.style('background-color', '#fcc860'); // Set button background color
  customFileButton.style('color', 'white'); //
  customFileButton.style('padding', '5px 10px'); 
  customFileButton.style('border', 'none'); 
  customFileButton.style('border-radius', '5px'); 
  customFileButton.style('cursor', 'pointer'); 
  customFileButton.style('margin', '5px 5px'); 
  customFileButton.position(2.5,0)
  fileInput.style('display', 'none'); // Hide default file input
  customFileButton.mousePressed(() => fileInput.elt.click()); // Trigger file input click event when custom button is clicked
  ...
  tool = createButton('Show Tools');  
  tool.style('background-color', '#c1cd97'); // Set button background color
  tool.style('color', 'white'); //
  tool.style('padding', '5px 10px'); 
  tool.style('border', 'none'); 
  tool.style('border-radius', '5px'); 
  tool.style('cursor', 'pointer'); 
  tool.style('margin', '5px 5px'); 
  tool.position(customFileButton.x + customFileButton.width + 8, 0);
  tool.attribute('disabled', ''); 
  tool.mousePressed(tbar);

  recorder = new Recorder(this);
  btn = createButton('Start Recording');
  btn.style('background-color', '#e18d96'); // Set button background color
  ...
  btn.position(tool.x + tool.width + 8,tool.y)
  ...
  timestampText = createP();
  timestampText.style('color', '#ededed'); // Set text color
  timestampText.position(width - 150, slider.y - 15); // Position the text next to the slider
  timestampText.style('font-family', 'Lato');
  ...
  playPauseButton.position(slider.x - 60, slider.y - slider.height /2 - 10); // Adjust the position as needed
  ...
  strokeSlider = createSlider(1,25,5);
  strokeSlider.style('width', '20%');
  strokeSlider.position(strokeLabel.x + 120, strokeLabel.y+strokeLabel.height);
  ...
  strokeColor.position(strokeColorLabel.x + 110, strokeColorLabel.y + strokeColorLabel.height - 2);
  ...
  minPartSlider = createSlider(0,40,3);
  minPartSlider.position(minPartSliderLab.x + 140, minPartSliderLab.y + minPartSliderLab.height);
  ...
  maxPartSlider = createSlider(0,40,7);
  maxPartSlider.position(maxPartSliderLab.x + 140, maxPartSliderLab.y + maxPartSliderLab.height);
  ...
  particleColor.position(particleColorLabel.x + 118, particleColorLabel.y + 17);
  disableAndHide(particleColor);   
  // particleColor.changed(play)

  shapeSelectLabel = createP('Waveform Shape');
  ...
  shapeSelect.position(135 + shapeSelectLabel.x,shapeSelectLabel.y+20);
  ...
  backgroundFileInput.position(backgroundLabel.x + 148, backgroundLabel.y+20)
...
function handleAudioFile(file) {
  if (file.type === 'audio') {
    ...
    alert('Invalid audio file uploaded!');
  }
} 
...
/**
 * controls blur for background image
 */
function checked() {
  play();
  if (checkbox.checked()) {
...
function draw() {
  ...
  if (shapeSelect.selected() == 'Circle') {
    ...
    timestampText.html(currentTime + ' / ' + duration);
  ...
  if (round(song.currentTime(),1) == round(song.duration(),1)) {
    // restart song
    song.jump(0);
  }   
}
